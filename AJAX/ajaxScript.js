...
function replaceTxt(){
    ...
    // specifiy what to get for ending
    request.onload = function(){
        document.getElementById("home").innerHTML;
    }

    // GET request to the file containing the above id
    request.open('GET', 'replace.html', true);

    // connect and add the text to the file where it is needed
    request.onreadystatechange = function () {
        if (this.readyState === 4 && this.status === 200) {
            document.getElementById('ending').innerHTML = request.responseText;
        }
    }
